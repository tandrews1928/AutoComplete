   ...
   /**
    ...
    public static class TrieAutocomplete implements Autocompletor {
        ...
        private void add(String word, double weight) {
            ...
            if (weight > root.mySubtreeMaxWeight)
                root.mySubtreeMaxWeight = weight;
    
            for (int c = 0 ; c < word.length(); c++){
                if (root.getChild(word.charAt(c)) == null)
                        root.children.put(word.charAt(c), new Node(word.charAt(c), root, weight));
                if (weight > root.mySubtreeMaxWeight || root.isWord == true)
                        root.mySubtreeMaxWeight = weight;
                root = root.getChild(word.charAt(c));
        ...
        public Iterable<String> topMatches(String prefix, int k) {
            if (k < 0) {
                throw new IllegalArgumentException("The number of words to be returned is negative.");
             }

            if (prefix == null) {
                throw new NullPointerException("The prefix is null.");
             }
            PriorityQueue<Node> nodeList = new PriorityQueue<Node>(new Node.ReverseSubtreeMaxWeightComparator());
            ArrayList<Node> matchList = new ArrayList<Node>();
            ArrayList<String> matchNames = new ArrayList<String>();
            Node root = myRoot;
            for (int c = 0 ; c < prefix.length(); c++){
                if (root.children.containsKey(prefix.charAt(c)))
                    root = root.getChild(prefix.charAt(c));
                else
            ...
            while (nodeList.size() > 0) {
                if (k <= matchList.size()) {
                    Collections.sort(matchList, Collections.reverseOrder());  //https://beginnersbook.com/2013/12/sort-arraylist-in-descending-order-in-java/
                    if(matchList.get(k-1).getWeight() > nodeList.peek().mySubtreeMaxWeight){
                        break;
                    }
                }    
                root = nodeList.poll();             
            ...
            if (k < matchList.size())
                for (int i = 0; i < k; i++) {
                    matchNames.add(matchList.get(i).myWord);
                }
        ...
        public String topMatch(String prefix) {
            Node root = myRoot;
            if (prefix == null) {
                throw new NullPointerException("The given prefix is null.");
            }
            for (int c = 0 ; c < prefix.length(); c++){
                if (root.children.containsKey(prefix.charAt(c)))
                    root = root.getChild(prefix.charAt(c));
            ...
            double largest = root.mySubtreeMaxWeight;

            if (root.isWord == true && root.myWeight == largest)
                    return root.myWord;

            while (root.isWord == false) {
                for (Node n : root.children.values())
                    if (n.mySubtreeMaxWeight == largest)
                        root = n;
            }
            return root.myWord;
        ...
        public double weightOf(String term) {
            ...
            for (int c = 0 ; c < term.length(); c++){
                if (root.children.containsKey(term.charAt(c)))
                    root = root.getChild(term.charAt(c));
                else
                    return 0.0;
            }
            return root.myWeight;
        }
